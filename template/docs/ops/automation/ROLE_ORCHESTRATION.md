...
- Future and active program parents must also declare `Authoring-Intent`. `executable-default` requires `## Child Slice Definitions`; `blueprint-only` is allowed only for explicit draft-only blueprints.
